...
@Autonomous(name = "AutoRed2Duck", group="Autonomous")
public class AutoRed2ExtraBlock extends LinearOpMode { //Locate Special Object
    ...
    public void runOpMode() {
        ...
        switch (ARMMOTOR) {
            ...
            case "REV20":
                ArmTicksPerRot = 560;
                ArmLevelTicks = new int[]{0, 1080, 980, 800, 180, 1140, 750, 550}; //0, 1000, 880, 750, 200}; // ArmLevelTicks[0] = ticks for down, ArmLevelTicks[1] = ticks for level 1, ...
                break;
            case "REV40":
...
//        LidOpen = false;
        ...
        crab(200, 0.5);
        turn(-90, 0.25);
        RunArm(ArmLevelTicks[Level], ArmSpeed);
        drive(400, 0.5);
        Lid.setPosition(0.8);
        LidOpen = true;
        sleep(500); //object falling out
...
//                break;
            case 0: // Drive towards bird bath and figure out where the bar code object
                if (!ObjDetected) { // if object not detected then run this
                    if (LeftTOFcm < BC_Dist) { // if all of a sudden it sees something then do this
